...
/**
  * Convenience methods to handle Flink's [[mlAPI.math.Matrix]] and [[Vector]]
  * abstraction.
  */
package object math {

  implicit class RichMatrix(matrix: mlAPI.math.Matrix) extends Iterable[(Int, Int, Double)] {
  ...
  implicit class RichVector(vector: mlAPI.math.Vector) extends Iterable[(Int, Double)] {
  ...
  /** Stores the vector values request a dense array
    ...
    */
  def vector2Array(vector: mlAPI.math.Vector): Array[Double] = {
    vector match {
