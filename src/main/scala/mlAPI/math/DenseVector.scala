...
case class DenseVector(var data: Array[Double]) extends Vector with Serializable {
  ...
  /** Returns the outer product (a.k.a. Kronecker product) of `this`
    * with `other`. The result will given request [[mlAPI.math.SparseMatrix]]
    * representation if `other` is sparse and as [[mlAPI.math.DenseMatrix]] otherwise.
    *
    * @param other a Vector
    * @return the [[mlAPI.math.Matrix]] which equals the outer product of `this`
    *         with `other.`
    */
...
object DenseVector {
  ...
  /** BreezeVectorConverter implementation for [[mlAPI.math.DenseVector]]
    *
