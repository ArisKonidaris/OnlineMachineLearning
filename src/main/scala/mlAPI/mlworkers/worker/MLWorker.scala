...
import mlAPI.parameters.{Bucket, LearningParameters, ParameterDescriptor}
...
abstract class MLWorker[ProxyIfc, QueryIfc]() extends NodeInstance[ProxyIfc, QueryIfc] {
  ...
  def configureWorker(request: Request): MLWorker[ProxyIfc, QueryIfc] = {
    ...
    if (config.contains("mini_batch_size")) {
      try {
        setMiniBatchSize(
          if (config("mini_batch_size").isInstanceOf[Double])
            config("mini_batch_size").asInstanceOf[Double].toInt
          else if (config("mini_batch_size").isInstanceOf[Int])
            config("mini_batch_size").asInstanceOf[Int]
          else 64
        )
      } catch {
